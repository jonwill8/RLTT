#!/usr/bin/env python3
"""
Statistical Significance Analysis Script (T-Test) for Code Benchmarks

This script performs paired t-tests to determine if differences between
GRPO and RLTT results are statistically significant on code benchmarks.
...
Usage:
    python run_ttest_analysis.py \
        --grpo_csv grpo_humaneval_*.csv \
        --rltt_csv rltt_humaneval_*.csv

    # Or auto-detect latest results for a benchmark:
    python run_ttest_analysis.py --benchmark humaneval
...
def load_csv_results(csv_path: str) -> List[float]:
    """Load pass@1 values from a repeated evaluation CSV file."""
    pass_at_1_values = []
    with open(csv_path, "r") as f:
        reader = csv.DictReader(f)
        for row in reader:
            # Handle both 'pass_at_1' and 'accuracy' column names
            if "pass_at_1" in row:
                pass_at_1_values.append(float(row["pass_at_1"]))
            elif "accuracy" in row:
                pass_at_1_values.append(float(row["accuracy"]))
            else:
                raise ValueError(f"CSV file {csv_path} missing 'pass_at_1' or 'accuracy' column")
...
def paired_ttest(grpo_scores: List[float], rltt_scores: List[float]) -> Dict:
    """Perform paired t-test between GRPO and RLTT results.

    Returns statistics including p-value, t-statistic, and effect size.
    """
    grpo = np.array(grpo_scores)
    rltt = np.array(rltt_scores)
...
def format_results(benchmark: str, results: Dict, grpo_csv: str, rltt_csv: str) -> str:
    ...
    output.append("")
    output.append("Performance (Pass@1):")
...
def run_analysis_for_benchmark(
    ...
    # Load data
    grpo_scores = load_csv_results(grpo_csv)
    rltt_scores = load_csv_results(rltt_csv)

    if len(grpo_scores) != len(rltt_scores):
        print(f"Warning: Mismatched sample sizes for {benchmark}")
        print(f"  GRPO: {len(grpo_scores)}, RLTT: {len(rltt_scores)}")
        # Use minimum length
        min_len = min(len(grpo_scores), len(rltt_scores))
        grpo_scores = grpo_scores[:min_len]
        rltt_scores = rltt_scores[:min_len]

    # Run t-test
    results = paired_ttest(grpo_scores, rltt_scores)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run t-test analysis comparing GRPO vs RLTT results on code benchmarks"
    )
    ...
    parser.add_argument("--results_dir", type=str,
                        default="/scratch/gpfs/OLGARUS/jw4199/looped_models/RLTT/repeated_eval_results_code",
    ...
    if len(all_results) > 1:
        ...
        print("=" * 70)
        print("")
        print(f"{'Benchmark':<15} {'GRPO Pass@1':<22} {'RLTT Pass@1':<22} {'p-value':<12} {'Sig.':<8}")
        print("-" * 79)
        ...
        for benchmark, result in all_results.items():
            ...
            print(f"{benchmark:<15} {grpo_str:<22} {rltt_str:<22} {p_str:<12} {sig_str:<8}")
