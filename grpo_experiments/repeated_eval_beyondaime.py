#!/usr/bin/env python3
"""
Repeated Evaluation Script for BeyondAIME Benchmark (GRPO)
...
Usage:
    python repeated_eval_beyondaime.py \
...
def get_beyondaime_answer(item: Dict[str, Any]) -> str:
    ...
    BeyondAIME answers can be integers or expressions, sometimes in \\boxed{} format.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Repeated evaluation on BeyondAIME for statistical significance testing")
    ...
    output_csv = os.path.join(
        args.output_dir,
        f"{args.method}_beyondaime_{exp_id}_{args.checkpoint}_n{args.num_runs}_{timestamp}.csv"
    )

    logger.info("=" * 60)
    logger.info("BeyondAIME Repeated Evaluation")
    logger.info("=" * 60)
