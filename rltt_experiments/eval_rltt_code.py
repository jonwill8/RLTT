#!/usr/bin/env python3
"""
Evaluate RLTT checkpoint on Code Generation benchmarks (HumanEval, MBPP).
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate RLTT checkpoint on code generation benchmarks")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to RLTT checkpoint directory (e.g., checkpoint-best)"
    )
    parser.add_argument(
        "--base_model_path",
        type=str,
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("RLTT Checkpoint Code Generation Evaluation")
    logger.info("=" * 60)
    ...
    # Check for FSDP checkpoint first
    ...
    with open(summary_file, "w") as f:
        f.write("RLTT Checkpoint Code Generation Evaluation Summary\n")
        f.write("=" * 50 + "\n\n")
