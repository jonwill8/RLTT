#!/usr/bin/env python3
"""
Evaluate Qwen3 baselines on competition benchmarks (AIME26, HMMT25).

This wraps evaluate_baseline.py with benchmark-aware defaults.
"""
import os
import json
import argparse
...
MODEL_PATHS = {
    "1.7b": "/scratch/gpfs/OLGARUS/jw4199/model_weights_path/Qwen3-1.7B",
    "4b": "/scratch/gpfs/OLGARUS/jw4199/model_weights_path/Qwen3-4B",
}


def normalize_model_size(size: str) -> str:
    size_norm = size.lower().replace(" ", "").replace("_", "")
    if size_norm in ("1.7b", "17b"):
        return "1.7B"
    if size_norm in ("4b", "4"):
        return "4B"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Qwen3 baselines on competition benchmarks"
    )
    parser.add_argument(
        "--benchmark",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="4B",
        help="Model size to evaluate (1.7B or 4B)",
    )
    parser.add_argument(
        "--model_path",
        type=str,
...
def main():
    ...
    if model_path is None:
        key = "1.7b" if model_size.startswith("1.7") else "4b"
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join(
            "./eval_output",
            f"qwen3_{model_size}_{args.benchmark}_prompt{args.max_prompt_length}_{few_shot_tag}_tokens{args.max_new_tokens}",
        )
    os.makedirs(args.output_dir, exist_ok=True)

    logger.info("=" * 60)
    logger.info(f"Qwen3 Baseline Evaluation - {bench_name}")
    logger.info("=" * 60)
    ...
    with open(summary_file, "w") as f:
        f.write(f"Qwen3 Competition Evaluation - {bench_name}\n")
        f.write("=" * 50 + "\n\n")
        f.write(f"Model: Qwen3-{model_size}\n")
