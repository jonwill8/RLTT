#!/usr/bin/env python3
"""
Repeated Evaluation Script for ARC-Challenge Benchmark (GRPO)
...
def load_dataset(filepath: str) -> List[Dict[str, Any]]:
    """Load ARC-Challenge test set from JSONL."""
    data = []
...
def format_mcqa_prompt(
    ...
    if use_few_shot:
        examples = FEW_SHOT_EXAMPLES.get("arc", [])
...
def main():
    parser = argparse.ArgumentParser(description="Repeated evaluation on ARC-Challenge for statistical significance testing")
    ...
    parser.add_argument("--test_file", type=str,
                        default="/scratch/gpfs/OLGARUS/jw4199/datasets/mcqa/arc_challenge.test.jsonl",
                        help="Path to ARC-Challenge test file")
    ...
    output_csv = os.path.join(
        args.output_dir,
        f"{args.method}_arc_c_{exp_id}_{args.checkpoint}_n{args.num_runs}_{timestamp}.csv"
    )

    logger.info("=" * 60)
    logger.info("ARC-Challenge Repeated Evaluation")
    logger.info("=" * 60)
