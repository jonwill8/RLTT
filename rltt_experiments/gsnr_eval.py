#!/usr/bin/env python3
"""
Gradient Signal-to-Noise Ratio (GSNR) Evaluation Script for RLTT Experiments

This script computes the GSNR metric as described in the paper to measure the quality
of the learning signal produced by the RLTT objective. GSNR is computed with respect
...
def compute_grpo_loss(
    ...
    """
    Compute GRPO/RLTT policy gradient loss.
...
def compute_gsnr_for_prompt(
    ...
    for r_idx, (response, advantage) in enumerate(zip(responses, advantages)):
        ...
        if include_intermediate_loops:
            # Multi-loop mode: Get logits from all UT loops
            outputs = model(
                input_ids=input_ids,
            ...
            # per_loop_logits is a list of [1, seq, vocab] tensors, one per loop
            per_loop_logits = outputs.per_loop_logits
            final_logits = outputs.logits  # [1, seq, vocab]

            if per_loop_logits is None or len(per_loop_logits) == 0:
                # Fallback: model doesn't support per-loop logits
                logger.warning("Model does not return per_loop_logits, falling back to final logits only")
                per_loop_logits = [final_logits]

            # Enable gradient retention on ALL loop logits
            for loop_logits in per_loop_logits:
                loop_logits.retain_grad()

            # Compute loss using the final loop logits (this is what RLTT trains on)
            ...
            # Get the model output - we need the logits from the final loop
            # The Ouro model's forward pass returns logits from all loops when configured
            outputs = model(
                input_ids=input_ids,
                attention_mask=attention_mask,
                use_cache=False,
...
def main():
    parser = argparse.ArgumentParser(
        description="GSNR evaluation for RLTT checkpoints"
    )
    ...
    if args.output_dir is None:
        args.output_dir = "/scratch/gpfs/OLGARUS/jw4199/looped_models/RLTT/gsnr_results/rltt"
    ...
    logger.info("=" * 70)
    logger.info("GSNR Evaluation for RLTT")
    logger.info("=" * 70)
    ...
    with open(summary_txt, "w") as f:
        f.write("GSNR Evaluation Summary (RLTT)\n")
        f.write("=" * 50 + "\n\n")
