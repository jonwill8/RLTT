#!/usr/bin/env python3
"""
Statistical Significance Analysis Script (T-Test)

This script performs paired t-tests to determine if differences between
GRPO and RLTT results are statistically significant.
...
Usage:
    python run_ttest_analysis.py \
        --grpo_csv grpo_math500_*.csv \
        --rltt_csv rltt_math500_*.csv
...
def format_results(benchmark: str, results: Dict, grpo_csv: str, rltt_csv: str) -> str:
    ...
    if results['significant_0.05']:
        output.append(f"  ✓ The difference IS statistically significant at α=0.05")
    else:
        output.append(f"  ✗ The difference is NOT statistically significant at α=0.05")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run t-test analysis comparing GRPO vs RLTT results"
    )
    ...
    parser.add_argument("--benchmark", type=str,
                        choices=["math500", "gsm8k", "aime24", "aime26", "beyondaime"],
    ...
    if args.grpo_csv and args.rltt_csv:
        ...
        benchmarks = ["math500", "gsm8k", "aime24", "aime26", "beyondaime"]
    ...
    if len(all_results) > 1:
        ...
        for benchmark, result in all_results.items():
            ...
            sig_str = "✓" if result['significant_0.05'] else ""
