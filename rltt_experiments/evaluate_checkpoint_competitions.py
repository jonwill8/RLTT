#!/usr/bin/env python3
"""
Evaluate RLTT checkpoint on competition math benchmarks (AIME26, HMMT25).
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate RLTT checkpoint on competition math benchmarks")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to test file (overrides benchmark default)"
    )
    parser.add_argument(
        "--max_samples",
        type=int,
...
def main():
    ...
    logger.info("=" * 60)
    logger.info(f"RLTT Checkpoint Evaluation - {bench_name}")
    logger.info("=" * 60)
    ...
    with open(summary_file, "w") as f:
        f.write(f"RLTT Checkpoint Evaluation Summary - {bench_name}\n")
        f.write(f"=" * 40 + "\n\n")
