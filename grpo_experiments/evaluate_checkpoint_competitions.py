#!/usr/bin/env python3
"""
Evaluate a GRPO checkpoint on competition math benchmarks (AIME26, HMMT25).

Usage:
    python evaluate_checkpoint_competitions.py \
...
def evaluate_vllm(
    ...
    for example in test_data:
        ...
        gold_answers.append(str(get_gold_answer(example)))
    ...
    results = []
    correct_count = 0
    for i, (output, gold_answer, problem) in enumerate(zip(outputs, gold_answers, problems)):
...
def evaluate_hf(
    ...
    for i in tqdm(range(0, len(test_data), batch_size), desc="Evaluating"):
        ...
        for j, (output, gold_answer, problem) in enumerate(zip(outputs, gold_answers, problems)):
...
def compute_metrics(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Compute accuracy metrics overall."""
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate GRPO checkpoint on competition math benchmarks")
    ...
    parser.add_argument("--test_file", type=str, default=None,
                        help="Path to test JSONL (overrides benchmark default)")
    ...
    # Load test data
    logger.info(f"Loading {bench_name} from {test_file}...")
    sys.stderr.flush()
