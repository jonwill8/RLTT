#!/usr/bin/env python3
"""
Baseline Evaluation Script for DeepSeek-R1-Distill-Qwen models on MCQA benchmarks.

Evaluates DeepSeek-R1-Distill-Qwen-1.5B and DeepSeek-R1-Distill-Qwen-7B on
ARC-Challenge, MMLU-STEM, and GPQA.

Uses the shared non_math_utils for answer parsing and checking.

Usage:
    python evaluate_baseline_mcqa.py \
...
class ModelConfig:
    """Model configuration."""
    model_name_or_path: str = "/scratch/gpfs/OLGARUS/jw4199/model_weights_path/DeepSeek-R1-Distill-Qwen-7B"
...
def evaluate_baseline_mcqa(config: BaselineMCQAEvalConfig) -> Dict[str, Any]:
    """Evaluate DeepSeek-R1-Distill-Qwen model on MCQA test dataset using vLLM.

    Returns:
        Dictionary containing metrics and detailed results.
    """
    logger.info("=" * 60)
    logger.info("DeepSeek-R1-Distill-Qwen Baseline Evaluation (MCQA)")
    logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate DeepSeek-R1-Distill-Qwen models on MCQA benchmarks")
